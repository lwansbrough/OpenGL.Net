...
namespace OpenGL
{
	...
	public partial class Gl : KhronosApi
	{
		...
		static Gl()
		{
			...
			//_HandleDevice = DeviceContextFactory.Create(_Handle);
			//_HandleDevice.IncRef();

			//// Create basic OpenGL context
			//IntPtr renderContext;

			//if (_HandleDevice is NativeDeviceContext)
			//	renderContext = CreateEglSimpleContext(_HandleDevice);
			//else
			//	throw new NotImplementedException(String.Format("{0} is not a supported device context", _HandleDevice.GetType()));

			//// Query OpenGL informations
			//if (_HandleDevice.MakeCurrent(renderContext) == false)
			...
			//	Egl._CurrentExtensions.Query((NativeDeviceContext)_HandleDevice);
			//}

			//// Before deletion, make uncurrent
			//_HandleDevice.MakeCurrent(IntPtr.Zero);
			//// Detroy context
			//if (_HandleDevice.DeleteContext(renderContext) == false)
		...
		/// <summary>
		/// Common handle used for getting device context without creating a new handle.
		/// </summary>
		private static IntPtr _Handle;

		/// <summary>
		/// Device context handle created from <see cref="_Handle"/>.
		/// </summary>
		private static IDeviceContext _HandleDevice;
